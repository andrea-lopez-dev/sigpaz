...
<body>
    <div class="header">
        <div class="logo">{{ $empresa_nombre ?? 'SIGPAZ' }}</div>
        <div>RUC: {{ $empresa_ruc ?? 'J-12345678-9' }}</div>
        <div>{{ $empresa_direccion ?? 'Dirección de la empresa' }}</div>
    </div>
    ...
    <div class="info-empresa">
        ...
        Razón Social: {{ $empresa_nombre ?? 'SIGPAZ' }}<br>
        RUC: {{ $empresa_ruc ?? 'J-12345678-9' }}<br>
        Dirección: {{ $empresa_direccion ?? 'Dirección de la empresa' }}
    </div>
    ...
    <div class="info-empleado">
        ...
        <table>
            ...
            <tr>
                ...
                <td>{{ $persona->tipoDocumento->nombre ?? 'C.I.' }}: {{ $persona->numero_documento ?? '' }}</td>
            </tr>
            <tr>
                <td><strong>Cargo:</strong></td>
                <td>{{ $cargo->nombre ?? $cargo->nombre_cargo ?? 'No especificado' }}</td>
            </tr>
            <tr>
                <td><strong>Fecha de Ingreso:</strong></td>
                <td>{{ isset($fecha_contrato) ? \Carbon\Carbon::parse($fecha_contrato)->format('d/m/Y') : '' }}</td>
            </tr>
            <tr>
                <td><strong>Número de Contrato:</strong></td>
                <td>{{ $numero_contrato ?? 'N/A' }}</td>
            </tr>
        </table>
    </div>
    ...
    <div class="clausula">
        ...
        <p>El trabajador se obliga a prestar sus servicios personales en el cargo de <strong>{{ $cargo->nombre ?? $cargo->nombre_cargo ?? 'el asignado' }}</strong>, bajo la dirección y supervisión del empleador.</p>
    </div>
    ...
    <div class="clausula">
        ...
        <p>El presente contrato entrará en vigencia a partir del {{ isset($fecha_contrato) ? \Carbon\Carbon::parse($fecha_contrato)->format('d/m/Y') : '' }}.</p>
    </div>
    ...
    <div class="footer">
        Documento generado por SIGPAZ - {{ $fecha_emision ?? now()->format('d/m/Y H:i:s') }}
    </div>
</body>
</html>
