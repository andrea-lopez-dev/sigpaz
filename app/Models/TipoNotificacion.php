...
class TipoNotificacion extends Model implements CatalogoInterface
{
    use HasFactory, HasBooleanAttributes;

    protected $table = 'tiponotificacion';
    protected $primaryKey = 'tipo_notificacion_id';
    ...
    {
        return $this->hasMany(Notificacion::class, 'tipo_notificacion_id', 'tipo_notificacion_id');
    }
    ...
    public function eliminarLogico(): bool
    {
        if (!$this->puedeEliminar()) {
            throw new \Exception('No se puede eliminar porque tiene notificaciones asociadas');
        }
    ...
    {
        return !$this->notificaciones()->exists();
    }
}